...
from tcheck import CheckbyPoison
import random
import geatpy as ea
from tproblem import  MyProblem
...
def draw_min_rect_rectangle(image,mask,i,j):
    ...
    y_max = y_coords.max()
    e = 50
    poi=[max(y_min-e,0),min(y_max+e,224-1),max(x_min-e,0),min(x_max+e,224-1)]
    # cut = (img)[poi[0]:poi[1], poi[2]:poi[3]]
    # plt.imshow(cut); plt.xticks([]) ; plt.yticks([]) ; plt.axis('off')
    #plt.savefig(forpscc+'cut.jpg',bbox_inches='tight',pad_inches=0)

    #plt.imsave("./tresult/cut"+str(i)+"_"+str(j)+".jpg",cut.cpu().numpy())
    #plt.imsave(forpscc+'cutt.jpg', cut.cpu().numpy())
    return poi
...
def BinaryMask(poi, i, j, img, net, newimgs,trans):
    ...
    x0, y0, x1, y1 = optlist[0],optlist[1], min(optlist[0]+optlist[2],224), min(optlist[1]+optlist[3],224)
    ...
    plt.imsave("./tresult/flag"+str(i)+"_"+str(j)+".jpg", newimg.permute(1,2,0).cpu().numpy())
    return newimg, clean
...
def GAN_patching_inputs(net,model, images,count, newimgs,trans):
    ...
    for j in range(len(images)):
      ...
      result, mask = overlay_mask(to_pil_image(img), to_pil_image(activation_map[0].squeeze(0), mode='F'), alpha=0.5)
      ...
      # plt.savefig('./tresult/trimask.jpg',dpi=43.5,bbox_inches='tight',pad_inches = -0.01)
