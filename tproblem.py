...
class MyProblem(ea.Problem):  # 继承Problem父类
    ...
    def Remove(self, x, y, delta_x, delta_y):
        ...
        for j in range( x, min(x+delta_x, 224)):
            for i in range(y, min(y+delta_y, 224)):
    ...
    def evalVars(self, x):  # 目标函数
        ...
        for i in range(np.size(x, 0)):
            ...
            flipn = self.CheckbyPoison(x0, y0, min(224,x0+delta_x), min(224,y0+delta_y), self.img)
            flag = (self.orilabel != predlabel)
            flag = flag + flipn * 0.1
            point.append(flag * 10000 - s*s/5018)
